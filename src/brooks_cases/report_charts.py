...
def plot_temporal_diagnosis(comparison: pd.DataFrame, output: Path) -> None:
    ...
    last_flagged = int(
        comparison.loc[comparison["common_mode_flag"], "interval_index"].max()
    )
    last_time = float(
        comparison.loc[
            comparison["interval_index"] == last_flagged,
            "mid_time_s",
        ].iloc[0]
    )
...
def plot_robustness_summary(
    ...
    palette = ListedColormap(
        plt.cm.Purples(np.linspace(0.28, 0.92, vmax - vmin + 1))
    )
    ...
    nominal_outline.set_path_effects(
        [
            path_effects.Stroke(linewidth=4.6, foreground=INK),
            path_effects.Normal(),
        ]
    )
    axis.add_patch(nominal_outline)
    ...
    axis.set_xticklabels(
        [f"{thresholds[index]:.2f}" for index in tick_positions]
    )
    ...
    axis.set_title(
        "Classification endpoint across decision settings",
        loc="center",
        fontsize=13.2,
        pad=13,
    )
    ...
    axis.annotate(
        "Nominal threshold\n0.05",
        xy=(nominal_col, len(baselines) - 0.58),
        xycoords="data",
        xytext=(0.72, 0.91),
